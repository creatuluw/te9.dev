...
const GET = async ({ url }) => {
  ...
  try {
    ...
    console.log("[OAuth] Verification - config after save:", {
      hasRefreshToken: !!verifyConfig?.refresh_token,
      lastSyncAt: verifyConfig?.last_sync_at
    });
    const redirectUrl = apiKey ? `/bookmarks?key=${apiKey}&auth=success` : `/bookmarks?auth=success`;
