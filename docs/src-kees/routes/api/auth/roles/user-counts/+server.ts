/**
 * GET /api/auth/roles/user-counts - Get user counts per role
 */
...
export const GET: RequestHandler = async ({ request }) => {
	try {
		...
		const result = await roleService.getUserCountsPerRole();
		...
		console.error('Get role user counts endpoint error:', error);
