<script lang="ts">
    ...
    import type { ThreadedMessage as ThreadedMessageType } from "$lib/utils/messageThreading";
    import type { PocketBaseUser } from "$lib/services/pocketbaseService";
    import { getReplyCount } from "$lib/utils/messageListHelpers";
    ...
    // Self-import replaces deprecated <svelte:self> for recursive rendering (Svelte 5 pattern)
    ...
    /**
     * ThreadedMessage component props
     * Renders a message within a threaded conversation,
     * with indentation, reply count badge, and collapse toggle.
     */
    interface Props {
        /** The threaded message node to display */
        threadedMsg: ThreadedMessageType;
        /** Indentation level for visual nesting (0 = root) */
        indentLevel?: number;
        /** Whether replies are initially collapsed */
    ...
    const message = $derived(threadedMsg.message);
    const replies = $derived(threadedMsg.replies);
    const replyCount = $derived(getReplyCount(threadedMsg));
...
<div style={indentStyle} class="threaded-message">
    <!-- Thread connector line -->
    {#if indentLevel > 0}
        <div class="relative">
            <div
                class="absolute left-0 top-0 bottom-0 w-px bg-zinc-200"
                ...
                <div class="flex items-center gap-2 mb-1">
                    ...
                    {#if hasReplies}
                        <button
                            ...
                            class="inline-flex items-center justify-center p-0.5 rounded hover:bg-zinc-100 transition-colors text-zinc-400 hover:text-zinc-600"
                    ...
                    {#if hasReplies}
                        <span
                            class="inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded text-xs font-medium font-inter bg-zinc-100 text-zinc-600"
                        >
                            {replyCount}
                            {replyCount === 1 ? "reactie" : "reacties"}
                        </span>
                    {/if}
                </div>
    ...
    {#if hasReplies && !collapsed}
        <div class="mt-2 space-y-2">
            {#each replies as replyNode}
                <ThreadedMessage
                    threadedMsg={replyNode}
