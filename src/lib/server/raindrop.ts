import { db } from './db';
import type { RaindropItem } from '$lib/types';
...
async function getStoredTokens(): Promise<{ refresh_token: string | null }> {
	const raindropConfig = await db.config.get('raindrop');
	return {
		refresh_token: (raindropConfig as Record<string, unknown>)?.refresh_token as string || null
	};
}

async function storeTokens(refreshToken: string, accessTokenValue: string, expiresIn: number): Promise<void> {
...
export async function fetchAllBookmarks(options?: { limit?: number; days?: number }): Promise<{ items: RaindropItem[]; hasMore: boolean; total: number }> {
	const token = await getAccessToken();
	const allBookmarks: RaindropItem[] = [];
	let page = 0;
	const perPage = 50;
	...
	while (true) {
		...
		const items = data.items as RaindropItem[];
...
export async function syncBookmarks(options?: { limit?: number; days?: number }): Promise<{ added: number; updated: number; skipped: number; hasMore: boolean; total: number }> {
...
export async function initiateOAuthFlow(code: string): Promise<{ refresh_token: string; access_token: string }> {
	const clientId = process.env.RAINDROP_CLIENT_ID;
	const clientSecret = process.env.RAINDROP_CLIENT_SECRET;
	const redirectUri = process.env.RAINDROP_REDIRECT_URI || 'http://localhost:5173/api/auth/callback';
